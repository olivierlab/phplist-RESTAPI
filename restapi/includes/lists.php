...
class Lists {
    ...
    static function listSubscribersAdd($id_list = 0, array $emails = array()) {
        if ($id_list == 0) {
        ...
        $subscribersClass = new Subscribers();
        $subscribers = $subscribersClass->subscribersGetByEmail($emails);

        $sql = "";
        foreach ($subscribers as $email => $id) {
            $sql .= " (" . $id . ", " . $id_list . ", now()),";
        }
        $sql = preg_replace("/,$/", "", $sql);

        try {
            $debutSQL = "INSERT INTO " . $GLOBALS['table_prefix'] . "listuser (userid, listid, entered) VALUES ";

            $db = PDO::getConnection();
            $stmt = $db->query($debutSQL . $sql);
            $stmt->execute();
            $db = null;
        } catch (PDOException $e) {
            Response::outputError($e);
    ...
    static function listSubscribersDelete($id_list = 0, array $emails = array()) {
        if ($id_list == 0) {
            $id_list = $_REQUEST['id_list'];
        }
        ...
        $sql = "DELETE FROM " . $GLOBALS['table_prefix'] . "listuser WHERE listid=" . $id_list . " AND userid in (SELECT ID FROM " . $GLOBALS['usertable_prefix'] . "user WHERE email IN ($tmp))";
        try {
            $db = PDO::getConnection();
            $stmt = $db->query($sql);
            $stmt->execute();
            $db = null;
            Response::outputMessage('Subscribers ' . $tmp . ' are unassigned from list ' . $id_list);
        } catch (PDOException $e) {
            Response::outputError($e);
        }
        die(0);
     ...
     * </p>
     */
    public static function listEmailDelete($list_id = 0, $email = '') {
        if ($list_id == 0)
            $list_id = $_REQUEST['list_id'];
        if ($email == '')
            $email = $_REQUEST['email'];
        $sql = "DELETE FROM " . $GLOBALS['table_prefix'] . "listuser WHERE listid=:list_id AND userid=(SELECT id FROM "
                . $GLOBALS['usertable_prefix'] . "user WHERE email=:email);";
        try {
            $db = PDO::getConnection();
            $stmt = $db->prepare($sql);
            ...
            $stmt->execute();
            $db = null;
            Response::outputMessage('Subscriber ' . $email . ' is unassigned from list ' . $list_id);
        } catch (PDOException $e) {
            Response::outputError($e);
        }
        die(0);
